...
export function VideoLibraryPage({ onOpen, onRefresh, onRequeue, revision = 0 }: VideoLibraryPageProps) {
  ...
  return <section className="standard-page page-frame library-page">
    ...
    <form className="library-filter-bar" onSubmit={submitSearch}><label className="library-search-field"><Search size={16} aria-hidden="true" /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="搜索标题或作者" aria-label="搜索视频库" /></label><select className="library-platform-filter" value={platform} onChange={(event) => { setPlatform(event.target.value); setPage(1); }} aria-label="平台筛选"><option value="">全部平台</option><option value="bilibili">哔哩哔哩</option><option value="douyin">抖音</option><option value="youtube">YouTube</option></select><button type="submit" className="secondary-button compact-button" disabled={loading}>搜索</button></form>
    <div className="library-body">
    {loading ? <div className="library-loading" role="status"><LoaderCircle className="spin" size={22} />正在加载视频库……</div> : error ? <div className="library-error" role="alert"><p>{error}</p><button type="button" className="secondary-button" onClick={() => void loadPage(page)}>重试</button></div> : videos.length === 0 ? <div className="empty-state"><VideoIcon size={36} strokeWidth={1.5} /><h2>{appliedQuery || platform ? "没有匹配的视频" : "视频库还是空的"}</h2><p>{appliedQuery || platform ? "请尝试调整搜索或筛选条件。" : "队列完成转录后，视频会自动出现在这里。"}</p></div> : <>
