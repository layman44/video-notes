        ...
        <div className="workspace-transcript-pane">
          <div className="workspace-pane-toolbar">
            ...
            <div className="workspace-toolbar-actions">
              {canTranslate ? <><div className="translation-switch" role="group" aria-label="字幕显示方式"><button type="button" className={translationMode === "original" ? "is-active" : ""} aria-pressed={translationMode === "original"} onClick={() => setTranslationMode("original")}>原文</button><button type="button" className={translationMode === "bilingual" ? "is-active" : ""} aria-pressed={translationMode === "bilingual"} onClick={() => setTranslationMode("bilingual")}>双语</button></div>{translationMode === "bilingual" && remainingTranslationCount > 0 ? <button type="button" className="translation-action" disabled={action !== null || segments.length === 0} onClick={() => void translate()}>{action === "translate" ? <LoaderCircle size={13} className="spin" /> : <Languages size={13} />}{translatedCount === 0 ? "开始翻译" : "继续翻译"}</button> : null}</> : null}
