        ...
        <div className="workspace-transcript-pane">
          <div className="workspace-pane-toolbar">
            ...
            <div className="workspace-toolbar-actions">
              {canTranslate ? <div className="translation-controls"><div className="translation-switch" role="group" aria-label="字幕显示方式"><button type="button" className={translationMode === "original" ? "is-active" : ""} aria-pressed={translationMode === "original"} onClick={() => setTranslationMode("original")}>原文</button><button type="button" className={translationMode === "bilingual" ? "is-active" : ""} aria-pressed={translationMode === "bilingual"} onClick={() => setTranslationMode("bilingual")}>双语</button></div>{translationMode === "bilingual" ? <div className="translation-status">{remainingTranslationCount > 0 ? <button type="button" className="translation-action" disabled={action !== null || segments.length === 0} onClick={() => void translate()}>{action === "translate" ? <LoaderCircle size={13} className="spin" /> : <Languages size={13} />}{translatedCount === 0 ? "开始翻译" : "继续翻译（剩余 " + remainingTranslationCount + " 段）"}</button> : <span>已全部翻译</span>}</div> : null}</div> : null}
          ...
          <div className="workspace-pane-hint"><span>{segments.length} 段 · 校正字幕{canTranslate ? " · 已翻译 " + translatedCount + "/" + segments.length : ""}</span><span>点击字幕跳转</span></div>
